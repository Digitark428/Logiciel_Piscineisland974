...
- Next.js 15.5 avec App Router, React 19, TypeScript strict, Tailwind CSS et Vitest.
...
- Les migrations SQL sont numérotées dans `supabase/migrations/` ; les dernières migrations appliquées sont `20260829080314_professional_backups` et `20260829080405_professional_backups_indexes`, alignées avec leurs fichiers locaux.
- Le fuseau IANA de chaque entreprise est conservé dans `workspaces.timezone` (défaut `Indian/Reunion`) et pilote la sauvegarde quotidienne. Les dates métier sans heure (`YYYY-MM-DD`) de la V1 réunionnaise restent calculées via `src/lib/utils/date.ts`, jamais avec `toISOString().slice(0, 10)`.
...
- Gestion des clients, contrats d'entretien hebdomadaires, entretiens ponctuels, planning, équipe, tâches, documents, sauvegardes, notifications internes et journal d'activité. Les revenus restent réservés au gérant : montant par entretien ponctuel, montant mensuel unique par contrat et synthèse financière admin-only du tableau de bord.
- L’entité historique « piscine » n’est plus une fonctionnalité utilisateur : aucune route, fiche, section, sélection, permission visible ni export ne permet de l’ajouter, la modifier ou la consulter. Les tables, relations et actions serveur historiques restent dormantes afin de ne supprimer aucune donnée ; seuls les replis internes d’adresse/GPS nécessaires aux anciens entretiens sont conservés. Les libellés métier légitimes tels que « Entretien piscine » restent autorisés.
...
- « Entre nous » est le fil interne privé d'une entreprise : publications texte/photos, hashtags cliquables, recherche serveur, réactions, commentaires et pagination par curseur. La galerie `/app/community/gallery` lit les médias existants sans les dupliquer et signe leurs URLs privées par lot ; sa lightbox globale est partagée avec le feed. Les nouvelles sources photo sont conservées sous leur format original en plus du WebP normalisé, afin que la sauvegarde puisse restituer l’original ; les lignes historiques utilisent leur meilleur fichier WebP disponible. Les sources mobiles restent validées côté serveur (orientation, dimensions, HEIC/HEIF) sans exposer la clé privilégiée. Les contenus et médias restent isolés par `workspace_id`, RLS et trigger d'intégrité tenant ; le bucket `community-media` reste privé.
...
- La barre latérale affiche le logo personnalisé de l’entreprise à côté de LETI lorsqu’il existe. Seul un administrateur peut l’ajouter, le remplacer ou le retirer ; les sources SVG, PNG et JPEG sont contrôlées, orientées, recadrées sur le contenu et normalisées côté serveur en WebP transparent 3:1 dans le bucket privé `workspace-assets`, puis signées pour les membres du workspace.
- « Mes entretiens » fournit une impression protégée de la semaine affichée et filtrée, du lundi au dimanche, au format A4 paysage. Le document ne contient que l’identité de l’entreprise, la semaine, la date de génération et, par passage, le jour/date, le nom du client et l’adresse.
- Les sauvegardes sont strictement réservées aux administrateurs. Une demande manuelle démarre immédiatement ; le répartiteur Vercel quotidien de 00:05 UTC crée une exécution par entreprise, puis le workflow durable attend le prochain 21 h dans son fuseau IANA avant la collecte paginée, le PDF, le XLSX et le ZIP privé assemblé en streaming. Cette architecture respecte la fréquence Cron du plan Hobby sans sacrifier l’heure locale. L’historique n’a aucune rétention automatique ; téléchargement et suppression passent par des actions serveur contrôlées. Les secrets, données Auth, to-do/planning personnels, anciennes données piscine et tables techniques sont exclus du catalogue utilisateur.
